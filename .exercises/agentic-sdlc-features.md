...
### Relevant Files
- `angular/src/app/pages/home/home.component.ts` — add favorites signal and toggle logic
- `angular/src/app/pages/home/home.component.html` — add toggle button and filter control
- `angular/src/app/pages/home/home.component.css` — style the active/inactive favorite state
- `angular/src/app/models/coffee.ts` — no change needed; favorites keyed by `coffee.id`

### Instructor Notes
- Good test of **localStorage** integration with Angular signals
- Students will see the agent create a `localStorage` helper and wire it into a computed signal
...
### Relevant Files
- `angular/src/app/pages/home/home.component.ts` — `searchTerm` signal, filtered computed
- `angular/src/app/pages/home/home.component.html` — search input with clear button
- `angular/src/app/pages/home/home.component.css` — input and clear-button styling

### Instructor Notes
- Demonstrates combining multiple signals (`searchTerm`, `roastFilter` from Task 6) into one computed
...
### Relevant Files
- `angular/src/app/app.routes.ts` — add `/coffees/:id` route
- `angular/src/app/services/coffee.service.ts` — add `getById(id)` method
- `angular/src/app/pages/` — new `coffee-detail/` component (agent creates this)
- `dotnet/src/Coffee2Live.App/Controllers/CoffeesController.cs` — `GetById` already exists; verify it returns all fields

### Instructor Notes
- Full-stack feature: agent touches both Angular and inspects (but likely does not modify) the .NET controller
- The star rating component is a good exercise in scoped component state vs. persisted state
- Students should verify the agent adds the route correctly in `app.routes.ts` and does not break the home route
...
- [ ] The Angular home page displays a "Top Picks" section above the main grid using this endpoint
...
- `dotnet/tests/Coffee2Live.Tests/UnitTest1.cs` — add test for recommendation logic
- `angular/src/app/services/coffee.service.ts` — add `getRecommended()` method
- `angular/src/app/pages/home/home.component.ts` — load recommended coffees
- `angular/src/app/pages/home/home.component.html` — "Top Picks" section
- `angular/src/app/pages/home/home.component.css` — distinct card style

### Instructor Notes
- Spans both layers — agent must coordinate changes across .NET and Angular
...
- `angular/src/app/app.routes.ts` — add `/admin/add-coffee` route
- `angular/src/app/pages/` — new `add-coffee/` component with reactive form
- `angular/src/app/services/coffee.service.ts` — add `add(coffee)` method
- `dotnet/tests/Coffee2Live.Tests/` — new test for duplicate rejection

### Instructor Notes
- Most complex feature in the lab — good capstone exercise
- Requires the agent to understand Angular Reactive Forms, HTTP POST with error handling, and .NET file I/O
