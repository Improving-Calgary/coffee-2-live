...
- Open the relevant CSS file (`blazor/wwwroot/css/app.css`)
...
4. Open `blazor/Models/Coffee.cs` and use **inline completion** to add `public decimal Price { get; set; }` to the model
5. Open `blazor/Pages/Home.razor` and use **inline completion** to display the price as a badge inside the card
...
2. Review the plan the AI produces (`@code` block changes, markup changes, no API changes needed)
3. Approve and let the AI apply changes to `blazor/Pages/Home.razor`
4. Use **inline completion** to fine-tune the CSS for the dropdown in `blazor/wwwroot/css/app.css`
...
**Steps:**
1. Open `blazor/Pages/Home.razor`
2. Use **inline completion** to add a `sortKey` field and a `SortedCoffees` computed property in the `@code` block
3. Use **inline completion** to add a `<select>` bound to `sortKey` in the markup section
4. If you get stuck, switch to **Ask mode** and ask: *"How should I update the Blazor component state and call StateHasChanged when sortKey changes?"*
...
2. Review generated files: a new `CoffeeDetail.razor` page with `@page "/coffees/{id}"` and a new method `GetByIdAsync()` in `CoffeeService.cs`
3. Verify the backend `GET /api/coffees/{id}` endpoint already exists in `CoffeesController.cs`
4. Use **inline completion** to add a "Back" link using `<a href="/">` to the detail page

**Modes to try:** Code mode for scaffolding, inline completion for the back-navigation link.
...
### Task 12 – Add a Blazor Loading Skeleton *(Frontend · Chat – Code Mode)*

**Goal:** Replace the plain "Loading…" text with an animated skeleton card while data fetches.

**Steps:**
1. Open **Code mode** and prompt: *"Replace the loading state in `blazor/Pages/Home.razor` with three skeleton placeholder cards using only CSS animations – no external libraries"*
2. Accept the changes to `Home.razor` and `blazor/wwwroot/css/app.css`
3. Use **inline completion** in `app.css` to tweak the animation timing or colors to match the coffee brand palette
